...
Экземпляр драйвера будет автоматически создан/удалён при подключении/отключении указанного оборудования к смарт-терминалу. При наличии нескольких подходящий драйверов пользователю будет предложен выбор.
...
`CASHDRAWER` - для принтеров ценников,
...
Можно задать actovity настроек, если оно требуется:
...
Указанная actovity должна находиться в текущем package и будет вызвана при первом подключении устройства или по нажатию на строчку с оборудованием в меню настроек оборудования.
...
MinSdkVersion должна быть на выше 22.

### 3. В реализации метода подключения к сервису для всех указанных в интент-фильтрах action'ов укажите соответствующие Binder'ы:
...
Для всех описываемых методов в случае невозможности выполнить требуемое действие (например, взвесить для метода getWeight) следует выбросить любой RuntimeException с текстовым человекочитаемым описанием проблемы.
...
#### `IVirtualDriverManagerService.Stub` - класс для управления драйверами виртуальных устройств: подключение и отключение устройств происходят здесь. Надо реализовать методы `addNewVirtualDevice`, `recreateNewVirtualDevice` и `destroy`.
...
Для устройств других категорий надо реализовать соответственно интерфейсы:
